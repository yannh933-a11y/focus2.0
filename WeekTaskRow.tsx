...
export function WeekTaskRow({ task, today, onOpen, onToggle, now }: Props) {
  ...
  const canStartTimer = task.timerEnabled && !task.completed && task.date === today && !activeTimer && !anotherTimerActive
  ...
  return <motion.div layout className={`week-task-row ${task.completed ? 'week-task-row-complete' : ''}`}>
    ...
    {canStartTimer
      ? <button onClick={() => startTimer(task.id)} className="focus-ring tap grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[var(--accent-soft)] text-[var(--accent)]" aria-label="Iniciar cronômetro"><Play size={15} fill="currentColor"/></button>
