...
function RoutineForm({ routine, onDone }: { routine?: RoutineTask | null; onDone:()=>void }) {
  ...
  const [draft,setDraft]=useState({
    ...
    notes:routine?.notes ?? '',
    timerEnabled:routine?.timerEnabled ?? false,
  ...
  return <form className="space-y-4" onSubmit={save}>
    ...
    <button type="button" onClick={()=>setDraft({...draft,timerEnabled:!draft.timerEnabled})} className="settings-row surface-soft w-full rounded-[16px] px-3.5"><span className="min-w-0 flex-1 text-left"><span className="block text-sm font-semibold">Cronômetro</span><span className="mt-0.5 block text-[11px] text-[var(--muted)]">Permitir cronômetro nas atividades desta rotina</span></span><span className={`toggle ${draft.timerEnabled?'toggle-on':''}`} aria-hidden="true"><span/></span></button>
