...
function AppContent(){
  ...
  useTheme()
  const change=(next:Tab)=>{setTab(next);localStorage.setItem('focus-tab',next);window.scrollTo({top:0,behavior:'auto'})}
  ...
  return <>
    ...
    <main className="min-h-dvh pb-[calc(112px+env(safe-area-inset-bottom))] pt-[calc(22px+env(safe-area-inset-top))] md:ml-64 md:pb-14 md:pt-10">
      <div className="mx-auto w-full max-w-6xl px-4 sm:px-6 lg:px-8"><Page/></div>
    </main>
    <ActiveTimerBar />
