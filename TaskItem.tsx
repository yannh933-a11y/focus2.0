...
export function TaskItem({ task, onOpen, now = Date.now() }: { task: TaskInstance; onOpen: () => void; now?: number }) {
  ...
  const timerEnabled = task.timerEnabled
  ...
  return <motion.div layout className={`task-row ${task.completed ? 'task-row-complete' : ''}`}>
    ...
    {timerEnabled && !task.completed && !isTimer && !anotherTimerActive
      ? <button aria-label="Iniciar cronômetro" onClick={() => startTimer(task.id)} className="focus-ring tap grid h-11 w-11 shrink-0 place-items-center rounded-[14px] bg-[var(--accent)] text-white"><Play size={16} fill="currentColor" /></button>
