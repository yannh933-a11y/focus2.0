...
export function TaskForm({ task, defaultDate, defaultRecurrence = 'none', onDone }: Props) {
  ...
  const [draft, setDraft] = useState<TaskDraft>(() => ({
    ...
    notes: task?.notes ?? '',
    timerEnabled: task?.timerEnabled ?? routine?.timerEnabled ?? false,
  }))
  ...
  return <form onSubmit={submit} className="space-y-4">
    ...
    <button type="button" onClick={() => change('timerEnabled', !draft.timerEnabled)} className="settings-row surface-soft w-full rounded-[16px] px-3.5">
      <span className="min-w-0 flex-1 text-left"><span className="block text-sm font-semibold">Cronômetro</span><span className="mt-0.5 block text-[11px] text-[var(--muted)]">Permitir iniciar um cronômetro nesta atividade</span></span>
      <span className={`toggle ${draft.timerEnabled ? 'toggle-on' : ''}`} aria-hidden="true"><span/></span>
    </button>
