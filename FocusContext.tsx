...
const seedState = (): AppState => {
  ...
  const routines: RoutineTask[] = [
    { id: uid(), title: 'Escola', categoryId: 'personal', time: '07:00', plannedDurationMinutes: 420, reminderMinutes: null, required: true, notes: '', timerEnabled: false, recurrence: 'weekly', daysOfWeek: [1,2,3,4,5], startDate: today, active: true, order: 10, createdAt, updatedAt: createdAt },
    { id: uid(), title: 'Academia', categoryId: 'gym', time: '17:00', plannedDurationMinutes: 90, reminderMinutes: 5, required: true, notes: '', timerEnabled: false, recurrence: 'weekly', daysOfWeek: [1,2,3,4,5], startDate: today, active: true, order: 30, createdAt, updatedAt: createdAt },
    { id: uid(), title: 'Estudo focado', categoryId: 'study', time: '15:30', plannedDurationMinutes: 60, reminderMinutes: 5, required: true, notes: '', timerEnabled: true, recurrence: 'weekly', daysOfWeek: [1,2,4,5], startDate: today, active: true, order: 20, createdAt, updatedAt: createdAt },
  ]
...
const migrateState = (saved: AppState): AppState => ({
  ...
  routines: (saved.routines ?? []).map((r, index) => ({ ...r, timerEnabled: r.timerEnabled ?? r.categoryId === 'study', order: r.order ?? (index + 1) * 10 })),
  tasks: (saved.tasks ?? []).map((t, index) => ({ ...t, timerEnabled: t.timerEnabled ?? t.categoryId === 'study', order: t.order ?? (index + 1) * 10 })),
...
export function FocusProvider({ children }: { children: ReactNode }) {
    ...
    setState(prev => {
      ...
      if (draft.recurrence === 'none') {
        return { ...prev, tasks: [...prev.tasks, {
          ...
          reminderMinutes: draft.reminderMinutes, required: draft.required, notes: draft.notes, timerEnabled: draft.timerEnabled, completed: false,
      ...
      const routine: RoutineTask = {
        ...
        required: draft.required, notes: draft.notes, timerEnabled: draft.timerEnabled, recurrence: draft.recurrence,
  ...
  const finishTimer = useCallback(() => setState(prev => {
    ...
    const timedTask = prev.tasks.find(t => t.id === timer.taskId)
    const session = { id: uid(), taskId: timer.taskId, startedAt, endedAt, durationSeconds }
    return {
      ...prev,
      ...
      studySessions: timedTask?.categoryId === 'study' ? [...prev.studySessions, session] : prev.studySessions,
      timer: { taskId: null, startedAt: null, accumulatedMs: 0, paused: true },
    }
  }), [])
